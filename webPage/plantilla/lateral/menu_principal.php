...
<ul class="sidebar-menu" data-widget="tree" style="font-size: 12px">
    <li class="<?php echo $class ?>"><a href="/consolidacion/principal"><i class="fa fa-home"></i> <span>Principal</span></a></li>
    <?php
    foreach ($listaMenu as $rsListado) {
        ?>
        <li  id="menuPrincipal<?php echo $rsListado['menu_id']; ?>" class="treeview" onclick="activar(<?php echo $rsListado['menu_id']; ?>, 1)">
            <a href="#">
            ...
            <ul class="treeview-menu">
                ...
                foreach ($mdlPacientes->obtenerSubMenu($rsListado['menu_id'], $usuPerfil, $usuId) as $subMenu) {
                    ?>
                    <li id="subMenu<?php echo $subMenu['menu_id'] ?>"   class="treeview" onclick="activar(<?php echo $subMenu['menu_id']; ?>, 2)">
