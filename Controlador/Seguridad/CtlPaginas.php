...
function guardarPaginas() {
    ...
    try {
        ...
        if ($parametosAfectados > 0) {
            $statusJson["success"] = "Pagina Creada correctamente.";
        } else {
            $statusJson["error"] = "El nombre de la pagina ya se existe";
        }
        } else {
            $updateAfectados = $mdlPaginas->actualizarPaginas($paginasVO);
            if ($updateAfectados > 0) {
                $statusJson["success"] = "Actualizado correctamente.";
            } else {
                $statusJson["error"] = "No se puede actualizar ";
            }
        }
